...
function hacer_login() {
    ...
    let clave_cifrada = md5($('#password').val());

    $.ajax({
        url: DIR_API + "/login",
        type: "POST",
        dataType: "json",
        data: { "username": usuario, "password": clave_cifrada }
        ...
        .done(function (data) {
            if (data.error) {
                ...
                $('#wrong-password').html("Incorrect username and/or password").css("visibility", "visible");
                $('#password').val("");
            }
            else {
                localStorage.setItem('token', data.token);
